...
#     help = "Fetch market data and predict every minute"
#
#     def handle(self, *args, **kwargs):
#         WINDOW = 30
#         SYMBOL = "BTCUSDT"
#
#         # ========== 1. 初始化数据库（回填过去 1 小时的市场数据） ==========
#         self.stdout.write("🔍 正在检查过去 1 小时市场数据是否完整…")
#         end_ts = int(time.time())  # 当前时间戳（秒）
#         start_ts = end_ts - 3600                              # 一小时前
#
#         # 用你封装的 API 回补缺失记录（你之前已经实现过）
...
#             )
#         self.stdout.write(f"✅ 数据回补完成，条数：{len(candles)}")
#
#         # ========== 2. 每分钟轮询预测 ==========
#         self.stdout.write("🚀 开始实时预测，每 60 秒轮询一次…")
#         last_ts = end_ts  # 初始化为当前时间点
#
#         while True:
#             # 拉取最新 candle（等同于轮询）
#             try:
#                 new_candle = fetch_klines(last_ts + 1, last_ts + 120)[-1]
...
#                 self.stdout.write(f"❌ 错误: {str(e)}，等待 5 秒后重试")
#                 traceback.print_exc()  # ✅ 显示完整 traceback
#                 time.sleep(5)
